...
| [doc-example](https://ike.network/doc-example/)[1] | You’re shipping a published document as the primary deliverable. No Java compile path; uses `<packaging>ike-doc</packaging>`. |
...
`example-project` releases after `ike-platform` (whose `ike-parent` this project consumes) and after `ike-docs` (whose `ike-doc-maven-plugin` is declared `extensions=true` by `ike-parent` to activate the documentation pipeline).
...
After inheriting, the project gets — for free — Java 25 compilation, GPG signing via Bouncy Castle, JaCoCo coverage, the AsciiDoc documentation pipeline, dependency-version management for the IKE ecosystem, and the `extensions=true` declarations for `ike-maven-plugin` and `ike-doc-maven-plugin`.
...
| Resource | URL |
| --- | --- |
| Source repository | [https://github.com/IKE-Network/example-project](https://github.com/IKE-Network/example-project)[3] |
| Cross-project issue tracker | [https://github.com/IKE-Network/ike-issues](https://github.com/IKE-Network/ike-issues)[4] |
| IKE Network landing page | [https://ike.network/](https://ike.network/)[5] |
| IKE Platform (parent POM, BOM, workspace plugin) | [https://ike.network/ike-platform/](https://ike.network/ike-platform/)[6] |
